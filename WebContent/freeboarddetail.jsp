...
BoardDTO board = bDao.selectBoardOne("freeboard", pidx);

List<BoardDTO> boardReplyList = bDao.selectBoardReplyList("freeboard_r", pidx);
%>
			...
			<div style="display: inline-block; width: 75%; vertical-align: top; text-align: left;">
				...
				<div style="padding: 30px;">
					...
					<div>
						...
						<input type="button" value="삭제" onclick="location.href = 'boardreplydelete.jsp?ridx=<%=rdto.getRidx()%>&type=freeboard&pidx=<%=rdto.getPidx()%>'">
					</div>
					<%} %>
				</div>
				<div style="padding: 30px;">
					<input type="button" value="댓글등록" onclick="location.href = './freeboardreplywrite.jsp?pidx=<%=board.getPidx()%>'">
				</div>
			</div>
		</div>
